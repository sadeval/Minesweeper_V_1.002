...
// Объявление констант для ячеек поля и мин
const char Closed_Cell = 0;
const char Exploded_Mine = 1;
const char Unchecked_Cell = 2;
...
// Открытие пустых клеток и раскрытие соседних
...
// Функция для открытия пустых клеток и раскрытия соседних (внешний интерфейс)
    ...
    do {
        ...
        case 2: {
            if (field_was_generated) {
                ...
                //cout << "Minesweeper\n\n";
                delete_field(field, a); // Очистка памяти после окончания игры
                field_was_generated = false; // Пометка о том, что поле больше не сгенерировано
            }
            else {
